...
public class QrCodeActivity extends ChoreStoryActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getStringExtra(getString(R.string.qr_code)).equals(getString(R.string.child)) &&
            !getIntent().getStringExtra(getString(R.string.token_identifier)).isEmpty()) {

            // Child is logging in with token generated by parents
            String childLoginToken = getIntent().getStringExtra(getString(R.string.token_identifier));

            // Identify this token by adding `:child` at the end of it
            childLoginToken = childLoginToken + getString(R.string.child_login_identifier);

            createQRCode(childLoginToken);

        } else if (getIntent().getStringExtra(getString(R.string.qr_code)).equals(getString(R.string.child))) {

            // Child is registering with token generated by parents
                ...
                childTokenQuery.enqueue(new Callback<SingleResponse<String>>() {
                    ...
                    public void onResponse(Call<SingleResponse<String>> call,
                        ...
                        if (response.isSuccessful() && response.body() != null && response.body().hasResponse()) {
                            ...
                            qrContent = qrContent + getString(R.string.child_register_identifier);
            ...
            // Parent is registering another parent with a generated token

            String token = tokenHandler.getToken(getApplicationContext());
            if (tokenHandler.isParentToken(token)) {
                Call<SingleResponse<String>> getParentRegistrationTokenQuery = retrofitInterface.get_parent_register_token(token);
                getParentRegistrationTokenQuery.enqueue(new Callback<SingleResponse<String>>() {
                    @Override
                    public void onResponse(Call<SingleResponse<String>> call, Response<SingleResponse<String>> response) {
                        if (response.isSuccessful() && response.body() != null && response.body().hasResponse()) {
                            String qrContent = response.body().getData();

                            qrContent = qrContent + getString(R.string.parent_register_identifier);
